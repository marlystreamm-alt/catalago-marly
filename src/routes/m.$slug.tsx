/** Página pública del menú de un negocio: /m/{enlace-del-negocio} */
...
export const Route = createFileRoute("/m/$slug")({
  ...
  head: ({ loaderData }) => {
    const name = loaderData?.business.name ?? "Menú no disponible";
    const title = loaderData ? `${name} — Menú digital` : "Menú no disponible";
    const description = loaderData
      ? `Consulta el menú de ${name} y haz tu pedido por WhatsApp en segundos.`
      : "Este menú no está disponible o el enlace cambió.";
    return {
      meta: [
        { title },
...
const priceLabel = (it: MenuItem) => (it.priceText.trim() ? it.priceText : money(it.price));
  ...
  return (
    <Shell>
      <div className="card-soft rounded-3xl border border-border bg-card p-8 text-center">
        ...
        <h1 className="text-xl font-bold">Este menú no está disponible</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Puede que el enlace haya cambiado o que el negocio esté pausado.
        </p>
    ...
    <Shell>
      ...
      {groups.length === 0 ? (
        <p className="rounded-3xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
          Este negocio todavía no publica platillos.
        </p>
      ) : null}
                ...
                {business.whatsapp ? (
                  <Button asChild size="sm" variant="outline" className="shrink-0 rounded-full">
                    <a
                      href={waLink(
                        business.whatsapp,
                        `Hola ${business.name}, quiero pedir: ${it.name} (${priceLabel(it)})`,
                      )}
                      target="_blank"
                      rel="noopener noreferrer"
